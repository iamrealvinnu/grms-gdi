...
function UpdateOpportunity() {
  ...
  const handleConvertToAccount = () =>{
      if(formOpportunityData.stageId &&
        opportunityStagesTypes[formOpportunityData.stageId] ==="Closed Won"
      ){
        navigate(`/addAccount/${opportunityId}`)
      } else {
      toast.error("Error Converting to Account: "+error.message)
          ...
          <div className="flex-1 bg-white rounded-lg shadow-md p-6">
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              {opportunityStagesTypes[formOpportunityData.stageId] === "Closed Won" && (
                <button
                type="button"
                className="w-full p-3 uppercase bg-emerald-600 text-white rounded-lg shadow hover:bg-green-700"
